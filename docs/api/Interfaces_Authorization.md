Configures the SDK for authorization wtih your server's [accessToken] token ) and automatically requests new tokens when server returns HTTP status `"401 Unauthorized"`.
